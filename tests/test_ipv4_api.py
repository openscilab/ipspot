...
def test_public_ipv4_whoer_net_success():
    result = get_public_ipv4(api=IPv4API.WHOER_NET, geo=True, timeout=40, max_retries=4, retry_delay=90, backoff_factor=1.1)
    assert result["status"]
    assert is_ipv4(result["data"]["ip"])
    assert set(result["data"].keys()) == DATA_ITEMS
    assert result["data"]["api"] == "whoer.net"
